...
@Component({
  selector: 'app-domicile-certificate',
  templateUrl: './domicile-certificate.page.html',
  styleUrls: ['./domicile-certificate.page.scss'],
})
export class DomicileCertificatePage implements OnInit {
  ...
  ngOnInit() {
    this.ValidateDomiclieCertificateForm();
  }

  ValidateDomiclieCertificateForm(): void {
    this.domicileForm = this.formBuilder.group(
      {
  ...
  get errorControl() {
    return this.domicileForm.controls;
  ...
  payAndSubmit() {
    ...
    if (!this.domicileForm.valid) {
      ...
      let domiclieCertificateParams = {
        ...
        nivash: this.domicileForm.value.address,
        full_name: this.domicileForm.value.fullName,
        ptype: this.domicileForm.value.fatherHusbandType,
        pname: this.domicileForm.value.fatherHusbandName,
        mother_name: this.domicileForm.value.motherName,
        dob: this.domicileForm.value.dob,
        place_of_birth: this.domicileForm.value.birthPlace,
        adhar_no: this.domicileForm.value.adharNo,
        mobile_no: this.domicileForm.value.mobileNo,
        address: this.domicileForm.value.permanentAddress,
        tahsil: this.domicileForm.value.tahsil,
        thana: this.domicileForm.value.policeStation,
        stay: this.domicileForm.value.residenceDuration,
        reason: this.domicileForm.value.needOfCasteCertificate,
        previous_issue: this.domicileForm.value.prevCasteCertificateSatus,
      ...
      console.log("domiclieCertificateParams  =", domiclieCertificateParams)
      //alert("domiclieCertificateParams =" + JSON.stringify(domiclieCertificateParams))
      ...
      if (this.shareData.memberDetails.UniqueId !== null) {
        ...
        if (this.shareData.walletBalance >= this.getFormName.form_price) {
          this.apiService.createDomicileCertificateForm(domiclieCertificateParams)
            .subscribe((data: any) => {
              if (data.action == "yes") {
                this.loadingService.hide();
                this.loadingService.showToast(data.msg, 2000, "success");
                console.log("domicile success ", data)
                setTimeout(() => {
              ...
              this.loadingService.hide();
              console.log("domicile fail ", error)
            })
